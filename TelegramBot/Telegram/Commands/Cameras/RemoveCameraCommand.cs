...
public class RemoveCameraCommand : ITelegramCommand
{
	private readonly ITelegramUsersRepo _usersRepo;
	private readonly ICamerasRepo _camerasRepo;

	private static ILog Log => LogProvider.Get().ForContext<RemoveCameraCommand>();

	public RemoveCameraCommand(ITelegramUsersRepo usersRepo, ICamerasRepo camerasRepo)
	{
	...
	public bool CanProcessUpdate(Update update) =>
		...
		update.CallbackQuery.Data.StartsWith("/removeCamera");

	public async Task ExecuteAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
	{
		Log.Info("Remove camera command received.");
		...
		var cameraToRemove = await _camerasRepo.FindCameraAsync(cameraId);
		if (cameraToRemove is null)
		{
			await AnswerQuery(
				botClient,
				update.CallbackQuery,
		...
		await _camerasRepo.DeleteCameraAsync(cameraToRemove.Id);
